...
| ⚙️ 本次同步做了什么 | 上游提交数、是否被 force push 重写、**冲突数**(只报计数)、对账与测试结论、**被丢的提交也要如实写出**。 |
| 📦 版本与回退 | 当前版本 + 回退命令 + `rollback.sh --list`。 |

> **冲突逐条细节刻意不进飞书**（消息要短）—— 它写在状态文件的 `conflicts_detail` 里，归档时渲染进
> `.sync-loop/runs/<轮次>/summary.md`。想知道"那次的冲突到底怎么解的"就翻本地归档，不在聊天记录里找。
