...
6. **调用本 loop 的脚本用仓库根的相对路径**:`bash scripts/upstream-sync/deploy.sh ...`(脚本自己会
   `cd` 到仓库根)。**命令被拒时不要换写法绕过** —— 照抄被拒命令原文写进状态文件与通知(见第 9 节)。
   注: 权限清单是全量放行 Bash(历史上有过细粒度规则, 但权限判断按"整条命令的首个程序"匹配,
   `echo "==="; git diff ...` 这类复合命令会被整条拒掉, 而 `A; B; C` 一行式是 agent 的天然习惯,
   细粒度清单必然长期漏报 —— 静默拒绝比放宽更危险)。
...
- 12 个文件有冲突, 逐个人工解决(逐条细节不进飞书, 见本地 `.sync-loop/runs/<轮次>/summary.md`)
...
- 同步动作与冲突**必须如实**: 被丢的提交、失败的测试、没做到的步骤, 全部写出来, 不要只报喜。
- **冲突逐条细节不写进飞书** —— 只留一行计数 + 指向本地记录。那些内容是给人事后复核用的,
  飞书消息要短; 细节写进状态文件的 `conflicts_detail` 字段(见下), 归档时由 `archive-run.sh`
  渲染进 `summary.md`。
- 没有新功能时就直说"本次上游以修复为主, 无新功能", 别硬凑。

### 状态文件的 `conflicts_detail`(冲突细节写这里, 不进飞书)

每个冲突文件一条, 写清"上游改了什么 / 我们怎么合", 这是事后复核的唯一依据:

```json
"conflicts": ["backend/app/api/watchlist.py", "..."],
"conflicts_detail": [
  "`backend/app/api/watchlist.py` — 上游新增「加入日期」「加入以来」两列的现算下发; 我们保留自定义财务列的接入, 接在它后面",
  "`frontend/src/pages/IndustryAnalysis.tsx` — 上游只接入共享列表导航(±3 行); 保留我们的基本面深历史与市值份额轨迹"
]
```
